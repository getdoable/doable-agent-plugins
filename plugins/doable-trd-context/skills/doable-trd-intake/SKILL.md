...
1. Preserve the first request for this feature verbatim in `feature.originalRequest`. On a later refresh of the same feature, keep that original value and append the new request verbatim to `feature.subsequentRequests`; never replace history or fabricate a clarification.
...
3. Before creating a new directory, inspect `.doable/features/*/doable-intake.json` when present. Reuse the existing feature directory and stable `featureId` when one record clearly matches the same product capability; increment `contextRevision`. If several records plausibly match, ask the user to select. If the local history is missing, reconstruct a new intake from current evidence and state that prior lineage could not be recovered.
4. Map repository boundaries before deep inspection. A monorepo is one repository; a multi-repo workspace has one entry per independent Git repository. Record each repository revision and dirty state locally. Never treat a common parent folder as a synthetic repository.
...
7. Treat Figma frames, screenshots, design documents, tickets, and runtime captures as first-class supplemental sources. Classify each as desired behavior only when the user makes it authoritative, current runtime when it depicts the tested environment, or reference context otherwise. Do not infer product intent from reference material.
...
9. Model the complete named feature domain without arbitrary flow, actor, or operation caps. Use `capabilities[]` to group related flows and assign every flow one `capabilityId`. Include only connected capabilities that belong to the feature. Trace downstream effects far enough to understand the feature's success semantics and record them as interfaces, rules, or observable effects. Do not promote an adjacent product or consumer to an executable flow unless the user explicitly includes it in scope or executing it is necessary to verify the named feature's core success oracle. Express executable operations as atomic actions and give every operation at least one terminal or error observable state; also capture initial, immediate, persistence, and failure states where they affect testing.
10. Record fixture requirements as state intent plus a repeatable preparation recipe when known. Actual provisioning is not required. Verify fixtures or runtime only in a safe local or disposable environment. Never request or copy credentials, real customer data, private URLs, or environment values.
...
14. Write canonical `doable-intake.json` using [assets/doable-intake.schema.json](assets/doable-intake.schema.json). Repository identities, revisions, dirty states, local locators, source anchors, and provenance remain only in this file.
15. Validate and render. Bind every declared repository:

   ```bash
   node <skill-directory>/scripts/validate-and-render.mjs \
     <path-to-doable-intake.json> \
     --repo REPO_FRONTEND=<frontend-root> \
     --repo REPO_BACKEND=<backend-root> \
     --out-dir <feature-directory>
   ```

   For exactly one repository, `--workspace-root <repository-root>` may replace `--repo`.
16. Fix every validation error and inspect warnings. Readiness requires zero unresolved material scope or oracle conflicts. Nonmaterial unknowns may remain and must be bounded. Self-scrub `doable-context.md` before handoff.
...
- **User Authority** contains the verbatim original request, ordered later same-feature requests, and exact clarification question/answer pairs.
...
Tell the user: create a suite in the Doable platform and upload `doable-context.md` to create the TRD.
