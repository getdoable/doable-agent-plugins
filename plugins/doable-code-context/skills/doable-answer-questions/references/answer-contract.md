...
- `statement`: one independently citable product proposition or one causally coherent state transition. Split unrelated lifecycle operations, validation families, outcomes, roles, and fixture facts. Do not submit a feature inventory as one finding.
...
- Include a remote finding only when it changes test scope, fixture/setup data, an observable oracle, or a material environment boundary. Discovered model fields, event taxonomies, webhooks, sibling mutations, filters, exports, notes, metadata controls, and internal mechanisms stay in the private ledger unless the frozen request names them or a selected behavior requires them. Delete executable-operation findings that still lack a grounded result or state change.
...
- `observableAnchors`: exact user-visible labels, messages, routes, states, protocol values, or product entities that directly support the finding's complete statement. Put the most representative public anchor first; Doable uses it as the compact visible quote. If no one anchor can represent the statement without becoming misleading, narrow or split the finding. Do not attach a nearby label merely because it appears in the same question or source. An `unknown` or `inference` finding may leave this empty when no exact observable anchor is established.
...
- Prefer a minimal progressive evidence graph: public entry or operation → owning handler/domain transition → exact observable outcome. Stop traversing a behavior family once this chain and its material boundary are established; do not collect every sibling call site, test, generated client, translation, or mutation for completeness.
